"""End-to-end evidence for one reviewed placement reference-designator revision."""
...
from boardgate.config.models import RuleProfile, RuleSeverity
...
FIXTURE = ROOT / "tests" / "fixtures" / "placement_reference_mismatch"
...
def _project_request(
    ...
    *,
    ...
    expected_reference: str = "C1",
    new_reference: str = "R2",
) -> ModificationRequest:
    ...
    return ModificationRequest(
        ...
        operation=SetPlacementReferenceDesignator(
            ...
            instruction="Rename the confirmed C1 placement reference to R2.",
        ),
    )
...
def _assert_only_requested_payload_changed(workspace: Path) -> None:
    for original in sorted(FIXTURE.iterdir()):
        ...
        if original.name == TARGET_PATH:
            assert before.count(b"C1") == 1
            assert after == before.replace(b"C1,7,5,90", b"R2,7,5,90")
        else:
            assert after == before


def _assert_reference_rule_resolved(validation_review: ReviewResult) -> None:
    match_result = next(
        item
        for item in validation_review.rule_results
        if item.rule_id.value == "bom_placement_reference_match"
    )
    assert match_result.outcome.value == "PASS"
    assert match_result.coverage.value == "FULL"
...
def test_base_fixture_has_the_exact_target_review_finding(tmp_path: Path) -> None:
    ...
    assert baseline_review.overall_status is ReviewStatus.READY_FOR_REVIEW
    assert len(baseline_review.findings) == 1
    baseline_finding = baseline_review.findings[0]
    assert baseline_finding.rule_id == "bom_placement_reference_match"
    assert "Placement-only references: C1." in baseline_finding.facts
    assert "BOM-only references: R2." in baseline_finding.facts
    assert any(
...
def test_cli_and_service_publish_the_same_complete_validated_revision(
    ...
    assert modification.operation.old_reference == "C1"
    assert modification.operation.new_reference == "R2"
    ...
    _assert_reference_rule_resolved(validation_review)
    assert not any(
        finding.rule_id == "bom_placement_reference_match"
...
@pytest.mark.parametrize(
    ("overrides", "code"),
    (
        ({"expected_reference": "R7"}, "AUTHORING_PLACEMENT_REFERENCE_NOT_FOUND"),
        ({"new_reference": "R1"}, "AUTHORING_PLACEMENT_NEW_REFERENCE_COLLISION"),
        ({"new_reference": "R22"}, "AUTHORING_PLACEMENT_NEW_REFERENCE_WIDTH"),
    ),
)
...
def test_cli_rejects_unsupported_token_forms_without_publication(
    ...
    shutil.copytree(FIXTURE, project)
    target = project / TARGET_PATH
    target.write_bytes(target.read_bytes().replace(b"C1,7,5,90", b'"C1",7,5,90'))
    request_path = tmp_path / "request.json"
    request_path.write_text(
        f"{canonical_json(_project_request(project))}\n",
        encoding="utf-8",
    ...
    assert "AUTHORING_PLACEMENT_REFERENCE_UNSUPPORTED" in result.output
    assert not output.exists()


def test_blocker_profile_transition_resolves_the_targeted_finding(
    tmp_path: Path,
) -> None:
    profile = load_rule_profile(RULES)
    match_setting = profile.rules.bom_placement_reference_match
    blocker_profile = profile.model_copy(
        update={
            "rules": profile.rules.model_copy(
                update={
                    "bom_placement_reference_match": match_setting.model_copy(
                        update={"severity": RuleSeverity.BLOCKER}
                    )
                }
            )
        }
    )
    blocker_rules = tmp_path / "blocker-rules.json"
    blocker_rules.write_text(
        f"{canonical_json(blocker_profile)}\n",
        encoding="utf-8",
    )
    baseline_output = tmp_path / "baseline-blocker"
    ...
    baseline_review = ReviewResult.model_validate_json(
        (baseline_output / "findings.json").read_text(encoding="utf-8")
    )
    ...
    assert modification.validation.overall_status is ReviewStatus.READY_FOR_REVIEW
    assert modification.validation.finding_ids == ()
