"""End-to-end evidence for one reviewed placement DNP-state revision."""
...
FIXTURE = ROOT / "tests" / "fixtures" / "placement_only_reference"
    ...
    *,
    ...
    return ModificationRequest(
        ...
        operation=SetPlacementDnpState(
            ...
            new_dnp=new_dnp,
            instruction="Mark the placement-only U1 reference DNP.",
        ),
    )
    ...
    for original in sorted(FIXTURE.iterdir()):
        ...
        if original.name == TARGET_PATH:
            assert after == before.replace(
                b"U1,12,10,0,Top,mm,0", b"U1,12,10,0,Top,mm,1"
            )
        else:
            assert after == before
...
def _assert_revision_evidence(
    ...
    assert modification.operation.reference == "U1"
    assert modification.operation.old_dnp is False
    assert modification.operation.new_dnp is True
...
def test_cli_and_service_publish_the_same_complete_validated_revision(
    ...
    marked = next(
        placement for placement in target_placements if placement.reference == "U1"
    )
    assert marked.dnp is True
    assert marked.position.x == 12.0
    assert marked.position.y == 10.0
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            {"expected_dnp": True, "new_dnp": False},
...
def test_cli_rejects_ambiguous_target_references(tmp_path: Path) -> None:
    ...
    target.write_bytes(target.read_bytes().replace(b"R1,5,5", b"U1,5,5"))
    request_path = tmp_path / "request.json"
...
def test_cli_rejects_unsupported_token_forms_without_publication(
    ...
    target = project / TARGET_PATH
    target.write_bytes(
        target.read_bytes().replace(b"U1,12,10,0,Top,mm,0", b"U1,12,10,0,Top,mm,no")
    )
    request_path = tmp_path / "request.json"
    request_path.write_text(
        f"{canonical_json(_project_request(project))}\n",
    ...
    assert "AUTHORING_PLACEMENT_DNP_UNSUPPORTED" in result.output
    assert not output.exists()


def test_cli_rejects_inverted_dnp_columns_without_publication(
    tmp_path: Path,
) -> None:
    project = tmp_path / "project"
    shutil.copytree(FIXTURE, project)
    target = project / TARGET_PATH
    target.write_bytes(target.read_bytes().replace(b",DNP\n", b",Fitted\n"))
    request_path = tmp_path / "request.json"
    request_path.write_text(
        f"{canonical_json(_project_request(project))}\n",
        encoding="utf-8",
    ...
    assert result.exit_code == 3
    assert "AUTHORING_PLACEMENT_DNP_COLUMN_UNSUPPORTED" in result.output
    assert not output.exists()
