...
GENERATION_OPERATION_KEYS = frozenset({("generate_two_layer_coupon", "1.0")})
...
class GenerateTwoLayerCoupon(VersionedModel):
    ...
    def require_realizable_geometry(self) -> Self:
        ...
        for hole in self.holes:
            ...
            if (
                2 * x_nm < pad_nm
                or 2 * x_nm + pad_nm > 2 * width_nm
                or 2 * y_nm < pad_nm
                or 2 * y_nm + pad_nm > 2 * height_nm
            ):
                msg = "each hole pad circle must fit inside the board outline"
                raise ValueError(msg)
            positions.append(
...
class GenerationResult(VersionedModel):
    ...
    operation: AppliedTwoLayerCouponGeneration
